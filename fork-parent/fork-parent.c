...
int fork_parent()
{
      ...
      case -1:
        switch (errno) {
          ...
          case EINTR:
            ...
            break;
          default:
            fprintf(stderr, "ERROR: Unable to read error messages: %s\n", strerror(errno));
            fflush(stderr);
            break;
        }
        break;
  ...
  if (waitpid(pid, &status, WNOHANG) > 0) {	/* got a child */
    if (WIFEXITED(status)) {
      ...
      fprintf(stderr, "FATAL: Server exited with code %d\n", result);
    } else if (WIFSIGNALED(status)) {
      fprintf(stderr, "FATAL: The server was killed by signal %d\n", WTERMSIG(status));
      fflush(stderr);
      result = EX_SOFTWARE;
#if 0
